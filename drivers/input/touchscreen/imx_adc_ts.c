...
static int ts_thread(void *arg)
{
	...
	while (input_ts_installed) {
		...
		if ((ts_sample.x_position != 0) || (ts_sample.contact_resistance == 0)) {
...
static int __init imx_adc_ts_init(void)
{
	...
	set_bit(EV_KEY, imx_inputdev->evbit);
	set_bit(BTN_TOUCH, imx_inputdev->keybit);
	set_bit(EV_ABS, imx_inputdev->evbit);
	set_bit(ABS_X, imx_inputdev->absbit);
	set_bit(ABS_Y, imx_inputdev->absbit);
	set_bit(ABS_PRESSURE, imx_inputdev->absbit);


	input_set_abs_params(imx_inputdev, ABS_X,
			0,
			MAX_12BIT,
			0, 0);
	input_set_abs_params(imx_inputdev, ABS_Y,
			0,
			MAX_12BIT,
			0, 0);
	input_set_abs_params(imx_inputdev, ABS_PRESSURE,
			0, MAX_12BIT, 0, 0);
