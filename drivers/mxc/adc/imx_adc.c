...
static int imx_adc_initialized = 0;
...
void imx_tsc_init(void)
{
	...
	reg |= TGCR_PDBTIME128 | TGCR_HSYNC_EN | TGCR_HSYNC_POL;
	__raw_writel(reg, tsc_base + TGCR);
				    ...
				    int wait_tsi)
{
	...
	touch_sample->x_position1 = ts_data_buf[5] >> 4;
	touch_sample->x_position2 = ts_data_buf[6] >> 4;
	touch_sample->x_position3 = ts_data_buf[7] >> 4;
			 ...
			 unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
	case IMX_ADC_INIT:
		if (imx_adc_initialized)
			pr_debug("init adc -> already done before!\n");
		else {
		pr_debug("init adc\n");
